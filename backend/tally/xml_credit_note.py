"""Credit Note (Sales Return) XML generation for Tally."""
import pandas as pd
from html import escape


def create_credit_note_tally_xml(df, company_name, sgst_ledger="Output SGST@2.5%", cgst_ledger="Output CGST@2.5%", igst_ledger="Output IGST@5%"):
    ...
    for ref_no, group in grouped:
      ...
      <LEDGERENTRIES.LIST>
       ...
       <AMOUNT>-{total_igst}</AMOUNT>
            ...
            if round_off != 0:
                round_deemed = 'Yes' if round_off > 0 else 'No'
                ledger_entries += f"""
      <LEDGERENTRIES.LIST>
...
{ledger_entries}
            ...
            errors.append(f"Credit note {ref_no}: {str(e)}")
...
def get_credit_note_template_csv():
    template_data = {
        'REFERANCE NO': ['CN/25-26/001', 'CN/25-26/001', 'CN/25-26/002'],
        'INVOICE DATE': ['05-01-2026', '05-01-2026', '10-01-2026'],
        ...
        'AMOUNT': [200.00, 200.00, 750.00],
        'SGST': [0, 0, 33.75],
        'CGST': [0, 0, 33.75],
        'IGST': [0, 0, 0],
        'TOTAL AMOUNT': [200.00, 200.00, 817.50]
    }
